...
def upgrade():
    ...
    for pid in payment_ids:
        ...
        for idx, (eid,) in enumerate(entry_rows):
            connection.execute(
                entry_table.update().where(entry_table.c.id == eid).values(index=idx)
            )
        # Transaction indexes
        ...
        for idx, (tid,) in enumerate(trans_rows):
            connection.execute(
                trans_table.update().where(trans_table.c.id == tid).values(index=idx)
            )
...
def downgrade():
    op.drop_constraint(
        "transaction_payment_id_index_key", "transaction", type_="unique"
    )
    op.drop_column("transaction", "index")

    op.drop_constraint(
        "payment_entry_payment_id_index_key", "payment_entry", type_="unique"
    )
